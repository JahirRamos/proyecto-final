...
void mostrarPrediccion24h(struct Registrocotocollao registros[], int numRegistros, const char* nombreZona) {
    ...
    printf("CO\t\t%d\t\t%.1f\t\t%d\t\t%s\n", 
           co2[0], tendenciaCO, predCO, evaluarContaminante(predCO, "CO2"));
...
void calcularPromediosHistoricos(struct Registrocotocollao registros[], int numRegistros, const char* nombreZona) {
    float sumasPM25 = 0, sumasPM10 = 0, sumasO3 = 0, sumasNO2 = 0, sumasSO2 = 0, sumasCO2 = 0;
    ...
    promedios[5] = (contCO2 > 0) ? sumasCO2 / contCO2 : 0;
    ...
    if (contCO2 > 0) {
        printf("CO2\t\t%.1f\t\t%d\t\t%s\t\t4.0\n", 
               promedios[5], contCO2, evaluarContaminante((int)promedios[5], "CO"));
    }
